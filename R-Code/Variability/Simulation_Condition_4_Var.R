...
results_df_ex4_nCW <- results_df_ex4_nCW %>%
...
# Create the new dataframe with the differences in absolute values
results_df_ex4_nCW_s <- results_df_ex4_nCW_s %>%
  mutate(p.value.error = c(NA, abs(diff(p.value)))) %>%
  mutate(estimate.error = c(NA, abs(diff(estimate)))) %>%
  mutate(std.error.error = c(NA, abs(diff(std.error))))
...
pval.error <- data.frame("p.value.error" = results_df_ex4_nCW$p.value.error) 
pval.error_s <- data.frame("p.value.error_s" = results_df_ex4_nCW_s$p.value.error) 

estimate.error <- data.frame("estimate.error" = results_df_ex4_nCW$estimate.error) 
estimate.error_s <- data.frame("estimate.error_s" = results_df_ex4_nCW_s$estimate.error) 

std.error.error <- data.frame("std.error.error" = results_df_ex4_nCW$std.error.error)
std.error.error_s <- data.frame("std.error.error_s" = results_df_ex4_nCW_s$std.error.error) 
...
dfs <- list(AcuteLoad_RMSE_ex4_nCW, 
...
library("writexl")
write_xlsx(table,"C:\\Users\\159479\\OneDrive - UTS\\Meta-Research\\2. Synthetic Data\\Experimental Paper\\1. Completed Article & Code\\Variability\\Results_SC4_Var.xlsx")
